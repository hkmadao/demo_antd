...
export const UserRoles: FC = () => {
  ...
  const handleRow = (record: TUserRole) => {
    return {
      onClick: async (_event: any) => {
        ...
        actionRef.current?.startEditable(record.idUserRole!);
      }, // 点击行
  ...
  const handleAddRow = () => {
    ...
    actionRef.current?.startEditable(userRoleNew.idUserRole as React.Key);
  };
  /**删除行 */
  const handleDelete = () => {
    if (editableKeys && editableKeys.length === 1) {
      const userRole = userRoles?.find(
        (t) =>
          t.action !== DOStatus.DELETED && editableKeys[0] === t.idUserRole,
      );
      if (userRole) {
